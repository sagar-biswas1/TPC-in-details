...
Interactive companion: [TCP on the Wire](tcp-on-the-wire.html) — packet simulator, header explorer,
state machine.
...
RTO    = SRTT + 4 × RTTVAR      (floor: 1 s in the RFC, 200 ms on Linux)
```
...
Each of these shows up in textbooks, diagrams and interview answers. Some are plain errors; most are
half-truths that hide the interesting part.

| Common belief | What's actually true |
|---|---|
| The app layer knows nothing about the layers below | True as abstraction, but it leaks: `TCP_NODELAY`, `SO_RCVBUF`, and a blocking `send()` when the peer's window closes are all TCP showing through. |
| Closing always takes four segments | Four logically, but the middle ACK and FIN are usually coalesced into one `FIN, ACK` — three segments in most real traces. RST closes in one. |
| One `send()` = one segment = one `recv()` | TCP is a **byte stream**. Write boundaries do not survive; "one request = one segment" is never guaranteed. |
| Header has 3 reserved bits + an NS flag | RFC 3540's layout. RFC 8311 (2018) retired NS: **Data Offset (4) · Reserved (4) · Control Bits (8) · Window (16)**. |
| 16 bits = 65,535 values | 16 bits = **65,536** values (0–65535). 65,535 *usable* ports only because port 0 is reserved. |
| A browser "doesn't use a port" | It doesn't *listen* on one. It still gets an ephemeral *source* port per connection; the four-tuple requires it. |
| The ISN is random | Unpredictable, not random. RFC 6528: a 4 µs timer plus a keyed hash of the four-tuple. |
| Ports stay the same on every segment | They swap with direction. Handshake segments 1 and 3: `src 50123 → dst 3000`; segment 2: `src 3000 → dst 50123`. |
| Nine or ten flags, including NS | Eight live bits, in header order: **CWR ECE URG ACK PSH RST SYN FIN**. |
| The server tells the client the window size | **Every segment carries its own sender's receive window**, both directions, continuously. |
| The advertised window is the limit on data in flight | That's flow control only. Real cap: **`min(rwnd, cwnd)`** — and cwnd has no header field at all. |
| The window can't exceed 65,535 bytes | The *field* can't. The *window* reaches 1 GiB via Window Scale. |

Most TCP explanations cover its *structure* thoroughly. What they skip is its *behaviour over time* —
congestion control, retransmission timers, TIME_WAIT — which is where nearly every real-world TCP
problem lives. A slow API is almost never a header-layout question and almost always an RTO, a closed
window, or a connection that should have been reused.
...
- **Timers** RTO = SRTT + 4·RTTVAR, floor 1 s (RFC) / 200 ms (Linux) · TIME_WAIT 2×MSL = 60 s on Linux · delayed ACK 40–200 ms
